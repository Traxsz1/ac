...
if game.PlaceId == 103754275310547 then 
    ...
    -- ปรับปรุงฟังก์ชันเช็คอาวุธใหม่
    local function findCurrentWeapon()
        local success, weaponText = pcall(function()
            return game:GetService("Players").LocalPlayer.PlayerGui.GUI:GetChildren()[28].Header.Text
        end)
        if success and weaponText then
            return weaponText:upper() -- แปลงเป็นตัวพิมพ์ใหญ่
        end
        return nil
    end

    -- ปรับปรุงฟังก์ชันเช็ค Perk ใหม่
    local function findCurrentPerk()
        local success, perkText = pcall(function()
            return game:GetService("Players").LocalPlayer.PlayerGui.GUI.Spin.Header.Text
        end)
        if success and perkText then
            return perkText:upper() -- แปลงเป็นตัวพิมพ์ใหญ่
        end
        return nil
    end
    ...
    local function hasDesiredWeapon(targetWeapons)
        ...
        if not currentWeapon then
            print("⚠️ ไม่สามารถหาอาวุธปัจจุบันได้")
            return false
        end
        
        for _, weapon in pairs(targetWeapons) do
            if string.find(currentWeapon, weapon:upper()) then
                print("✅ ได้อาวุธที่ต้องการแล้ว:", weapon, "(ปัจจุบัน:", currentWeapon, ")")
                return true
            end
        end
        print("🔄 อาวุธปัจจุบัน:", currentWeapon, "ยังไม่ใช่ที่ต้องการ")
        return false
    end

    local function hasDesiredPerk(targetPerks)
        local currentPerk = findCurrentPerk()
        if not currentPerk then
            print("⚠️ ไม่สามารถหา Perk ปัจจุบันได้")
            return false
        end
        
        for _, perk in pairs(targetPerks) do
            if string.find(currentPerk, perk:upper()) then
                print("✅ ได้ Perk ที่ต้องการแล้ว:", perk, "(ปัจจุบัน:", currentPerk, ")")
                return true
            end
        end
        print("🔄 Perk ปัจจุบัน:", currentPerk, "ยังไม่ใช่ที่ต้องการ")
        return false
    end
    ...
    local function autoSpin()
        ...
        -- ตรวจสอบอาวุธและ Perk ก่อน
        local hasWeapon = hasDesiredWeapon(targetWeapons)
        local hasPerk = hasDesiredPerk(targetPerks)
        
        if hasWeapon and hasPerk then
            print("🎉 มีทั้งอาวุธและ Perk ที่ต้องการแล้ว! ข้าม Auto Spin")
            _G.AutoSpin = false
            return
        end
        ...
        while _G.AutoSpin do
            ...
            -- สุ่มอาวุธเฉพาะเมื่อยังไม่มี
            if not hasDesiredWeapon(targetWeapons) then
                if not spinWeapon() then
                    if checkMoney() >= _G.MinMoneyForSpin then
                        print("🔄 ครบรอบการสุ่มอาวุธ")
                    else
                        break
                    end
                else
            ...
            -- สุ่ม Perk เฉพาะเมื่อยังไม่มี
            if not hasDesiredPerk(targetPerks) then
                if not spinPerk() then
                    if checkMoney() >= _G.MinMoneyForSpin then
                        print("🔄 ครบรอบการสุ่ม Perk")
                    else
                        break
                    end
                else
